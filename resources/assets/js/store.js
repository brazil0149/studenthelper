...
export const store = new Vuex.Store({
      ...
      mutations: {
            ...
            update_post_favs (state, payload)
            {
                  var post = state.posts.find( (p) => {
                        return p.id === payload.id
                  })

                  post.favs.push(payload.like)
            },
            unfav_post(state, payload) {
                  
                  var post = state.posts.find((p) => {
                        return p.id === payload.post_id
                  })

                  var favorite = post.favs.find( (l) => {
                        return l.id === payload.favorite_id
                  })

                  var index = post.favs.indexOf(like)

                  post.favs.splice(index, 1)
            }
      }
})